...
function bbr_theme(&$existing, $type, $theme, $path){
  $hooks = array();
   // Make user-register.tpl.php available
  $hooks['user_register_form'] = array (
     'render element' => 'form',
     'path' => drupal_get_path('theme','bbr'),
     'template' => 'user-register-form',
     'preprocess functions' => array('bbr_preprocess_user_register_form'),
  );
  return $hooks;
}

function bbr_preprocess_user_register_form(&$vars) {
  $args = func_get_args();
  array_shift($args);
...
/*
function bbr_preprocess_page(&$vars) {
...
function bbr_preprocess_node(&$vars, $hook) {
 ...
 */
function bbr_menu_link__menu_block($variables) {
  return theme_menu_link($variables);
}


function bbr_pager($vars) {
  $vars['tags'] = array('','','','','');
 ...
 * @param $trail
 */
function bbr_superfish_active_trail_alter(&$trail) {
 ...
 */
function bbr_preprocess_superfish_menu_item(&$variables) {
