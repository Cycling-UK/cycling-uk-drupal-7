...
(function($) {
  ...
  var addressfieldAutocompleteResetMap = function(o) {
    ...
    if (map) {
      ...
      var marker_posistion = {
        'lng': parseFloat(o.data('widget').find('.longitude').val()),
        'lat': parseFloat(o.data('widget').find('.latitude').val())
      };

      // If still not set use center of the map.
      if (marker_posistion.lng == 0 && marker_posistion.lat == 0) {
        marker_posistion = center;
      }
      ...
      if (marker) {
        marker.setPosition(marker_posistion);
      }
    }
