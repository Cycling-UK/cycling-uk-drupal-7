...
function bbrxl_theme(&$existing, $type, $theme, $path){
  $hooks = array();
   // Make user-register.tpl.php available
  $hooks['user_register_form'] = array (
     'render element' => 'form',
     'path' => drupal_get_path('theme','bbrxl'),
     'template' => 'user-register-form',
     'preprocess functions' => array('bbrxl_preprocess_user_register_form'),
  );
  return $hooks;
}

function bbrxl_preprocess_user_register_form(&$vars) {
  $args = func_get_args();
  array_shift($args);
...
function bbrxl_preprocess_page(&$vars) {
...
function bbrxl_preprocess_node(&$vars, $hook) {
...
function bbrxl_menu_link__menu_block($variables) {
  return theme_menu_link($variables);
}

function bbrxl_pager($vars) {
  $vars['tags'] = array('','','','','');
